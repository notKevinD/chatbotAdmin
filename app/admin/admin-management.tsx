...
import { AdminUserRow } from "@/app/admin/types";
    ...
    <section className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">Tambah, lihat, atau nonaktifkan akun admin panel ini.</p>
        </div>
        <button
        ...
        <ul className="divide-y divide-slate-100">
          {admins.map((admin) => (
            <li key={admin.id} className="p-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              ...
              <button
                className="shrink-0 px-3 py-1.5 border border-slate-300 rounded-lg text-xs font-semibold hover:bg-slate-50"
