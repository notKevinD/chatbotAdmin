    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="report-bar bg-white p-5 border border-slate-200 rounded-xl shadow-sm">
        <div className="report-row flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <span className="report-label font-semibold text-slate-800 text-sm">Filter Riwayat Chat</span>
        ...
        {range === "custom" && (
          <div className="custom-date-panel grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 bg-slate-50 border border-slate-200 rounded-lg mb-4 items-end">
            <div className="field compact-field flex flex-col gap-1">
              <label className="text-xs text-slate-600 font-medium" htmlFor="chatStartDate">Dari tanggal</label>
              <input
            ...
            <div className="field compact-field flex flex-col gap-1">
              <label className="text-xs text-slate-600 font-medium" htmlFor="chatEndDate">Sampai tanggal</label>
              <input
                className="input border border-slate-300 rounded-lg p-2 text-sm bg-white text-slate-800"
                id="chatEndDate"
                type="date"
            ...
            <button className="button bg-slate-800 text-white text-sm py-2 rounded-lg hover:bg-slate-950 transition-all font-medium" onClick={() => loadSessions(1)} type="button">
              Terapkan
            </button>
          </div>
        )}
        ...
        <div className="report-actions flex flex-col sm:flex-row gap-2 mt-2">
          ...
          <div className="flex gap-2">
            <button className="px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-all shadow-sm" onClick={() => loadSessions(1)} type="button">
              Cari
            </button>
            <button
      ...
      <section className="table-wrap bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500">Klik salah satu berkas session untuk mengevaluasi detail obrolan.</p>
        </div>
          ...
          {sessions.map((session) => (
            ...
            >
              <span className="session-main">
                <span className="block font-semibold text-slate-800 text-sm">{session.visitorName || "Tanpa nama"}</span>
                <span className="block text-xs font-mono text-slate-400 mt-0.5">{session.sessionId}</span>
              </span>
              <span className="session-meta flex flex-wrap sm:flex-col items-start sm:items-end gap-x-3 text-xs text-slate-500">
                <span className="font-medium text-slate-700">{session.total} pertanyaan</span>
                <span>{formatIndonesianDateTime(session.lastSeen)}</span>
                {session.visitorSchoolOrigin && (
                  <span className="px-1.5 py-0.5 bg-slate-100 text-slate-700 rounded text-[10px] mt-0.5 font-medium">{session.visitorSchoolOrigin}</span>
                )}
              </span>
            </button>
          ))}
          {!sessions.length && <div className="p-8 text-center text-sm text-slate-400">Belum ada data session.</div>}
        </div>
      ...
      {isDetailOpen && selectedSession && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div aria-modal="true" className="bg-white rounded-xl shadow-2xl w-full max-w-3xl h-[85vh] flex flex-col overflow-hidden border border-slate-200" role="dialog">
            
            {/* Header Modal */}
            <div className="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-indigo-600">Jendela Log Dialog</span>
                <h3 className="text-base font-bold text-slate-800 truncate max-w-md sm:max-w-xl">Sesi: {selectedSession}</h3>
              </div>
            ...
            {pairs && pairs.length > 0 && (pairs[0]?.visitorName || pairs[0]?.visitorPhoneNumber || pairs[0]?.visitorSchoolOrigin) && (
              <div className="bg-indigo-50/50 p-4 border-b border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-3 shrink-0 text-xs text-slate-700">
                ...
                <div><span className="text-slate-400 block font-medium">Asal Institusi/Sekolah</span><strong>{pairs[0]?.visitorSchoolOrigin || "-"}</strong></div>
              </div>
            )}
              ...
              {pairs && pairs.map((pair, index) => (
                <div key={`${pair.createdAt}-${index}`} className="space-y-1.5 animate-in fade-in duration-200">
                  <div className="flex justify-between items-center px-1">
                    ...
                    {pair.isFallback && (
                      <span className="px-1.5 py-0.5 bg-rose-100 text-rose-700 rounded text-[10px] font-semibold uppercase">Jawaban Bermasalah</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col items-end max-w-[85%] ml-auto">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 text-xs text-slate-400">
                      ...
                      {getContextItems(pair.context).length ? (
                        <button className="text-indigo-600 hover:text-indigo-800 font-semibold underline bg-transparent" onClick={() => setContextPair(pair)} type="button">
                          • Lihat Context RAG
                        </button>
                      ) : null}
                    </div>
